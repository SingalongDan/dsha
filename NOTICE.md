...
- **本项目（DSHA 自有代码）：`AGPL-3.0-only`**（SPDX 标识；LICENSE 为 AGPL-3.0 全文）—— 这是**项目所有者的选择**，
  与上游 MIT 无冲突（宽松许可可被强 copyleft 项目包含；反向则不成立）。
- 选 `-only` 而非 `-or-later` 是刻意的：不允许他人按**未来的** AGPL 版本（可能更宽松）重新许可，取最严格解释。
  ⚠️ 若将来要改为更宽松的许可（如 MIT），**上游不构成任何障碍**；但**许可证变更需所有者决定**，且发布后变更会影响已有用户，应在首次对外发布前确定。
